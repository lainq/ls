...
export const directoryExists = (directory: string, isDirectory:boolean): boolean => {
    try {
      const exists: boolean = existsSync(directory);
      const isDir: boolean = isDirectory ? statSync(directory).isDirectory() : true
      return exists && isDir;
    } catch (exception) {
      return false;
    }
...
const validateDirectory = (results:ArgumentParserResult):ArgumentParserResult => {
